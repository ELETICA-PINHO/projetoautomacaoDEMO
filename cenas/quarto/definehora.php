...
if(isset($_POST['opcao']) && !empty($_POST['opcao'])){
    ...
    if($opcao == 'ligar'){
        $sql = "UPDATE status_despertador_quarto SET hora_ligar = '$alarme',  status_ligar = '$botao' ";
         $sql = $pdo->query($sql);

    }

    if($opcao == 'desligar'){
        $sql = "UPDATE status_despertador_quarto SET hora_desligar = '$alarme',  status_desligar = '$botao' ";
         $sql = $pdo->query($sql);
...
$sql = "SELECT * FROM status_despertador_quarto";
$sql = $pdo->query($sql);
if($sql->rowCount () > 0){
    $sql = $sql->fetch();
